...
energy_diff_coefficient=100
#用來畫圖 儲存每個時間點theta跟w的值
theta_box = []
w_box = []
moment=[]
#設置偽隨機數種子 確保下次能找到相同的路徑走法
np.random.seed(1)
tf.set_random_seed(1)

action_space=['forward','dormant','backward','throw']
image = svgwrite.Drawing('animation2.svg', size=(1024,1024))
circle=image.add(image.circle((100+R,0),5))
image.add(image.line((0, 400-R), (1024, 400-R), stroke=svgwrite.rgb(0, 0, 0, '%')))
...
def shoot_the_ball2(x,y,v,theta,print_this_episode):
    ...
    if print_this_episode:
        ...
        while tmp_t<=math.floor(time) and x+v*math.cos(theta)*tmp_t<1000:
            ...
            circle.add(image.animateMotion(path=path,dur=str(delta_t)+"s", begin=str(i*delta_t)+"s",fill="freeze"))
            i=i+1
            tmp_t=tmp_t+delta_t
...
def do_action(action,print_this_episode):
    global L_m,L,theta,w,g,delta_t,R,m,k,k2,i,max_x_num,max_x_val,action_space,count,x_box,y_box,energy_coefficient,last_coefficient,energy_diff_coefficient
    ...
    if action==3:
        ...
        L=0
        w=w+(L-R*math.sin(theta)*m*g)/(m*R*R)-k2*w
    else:
    ...
    if print_this_episode:
        ...
        circle.add(image.animateMotion(path=path,dur=str(delta_t)+"s", begin=str(i*delta_t)+"s",fill="freeze"))
        image.add(image.line((0, -R+400), (1024, -R+400), stroke=svgwrite.rgb(0, 0, 0, '%')))
    ...
    return np.array([math.sin(theta),math.cos(theta),w]),energy_coefficient*energy,False
# Basic network off-policy
class BasicNetwork:
    def __init__(
            self,
            n_actions,
    ...
    def _build_net(self):
        ...
        self.q_target = tf.placeholder(tf.float32, [None, self.n_features], name='Q_target')  # for calculating loss
        with tf.variable_scope('eval_net'):
            ...
            with tf.variable_scope('l2'):
                w2 = tf.get_variable('w2', [n_l1, self.n_features], initializer=w_initializer, collections=c_names)
                b2 = tf.get_variable('b2', [1, self.n_features], initializer=b_initializer, collections=c_names)
    ...
    def learn(self):
        ...
        q_target[batch_index] = reward + self.gamma * np.max(q_next, axis=1)
...
def throw_ball(RL):
    global L_m,L,theta,w,g,delta_t,R,m,k,i,max_x_num,max_x_val,action_space,count
    step = 0
...
if __name__ == "__main__":
    ...
    title('x axis is theta, y axis is dot_theta')
    throw_ball(RL)
